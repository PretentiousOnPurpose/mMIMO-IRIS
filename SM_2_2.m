...
%     DATA_SYMS_IND = [2;3;4;5;7;8;9;10];
    ...
    if (SIM_MOD == 1)
        ...
        n = (1/sqrt(2*1*N)) * (randn(size(ul_tx_data)) + 1i * randn(size(ul_tx_data)));
...
%   
    ...
    for iter_syms = 1: (numSyms - 1)
        if (any(DATA_SYMS_IND == iter_syms))        
            dl_tx_data_buff(nonZeroSubcarriers, iter_syms) = modData((iter_data_syms - 1) * numDataCarriers + 1: iter_data_syms * numDataCarriers, 1);
    ...
    dl_tx_data = [zeros(iris_pre_zeropad, 1); prmSeq; dl_tx_data_buff(:); zeros(iris_post_zeropad, 1)];
...
%         h = (1/sqrt(2)) * (randn(1, 1) + 1i * randn(1, 1));
        ...
        dl_rx_data = h .* cfo_phase .* dl_tx_data + n;    
    end
    ...
    for iter_syms = 1: (numSyms - 1)
        dl_rx_data_buff(:, iter_syms) = dl_rx_data((iter_syms - 1) * (N + CP) + CP + 1: iter_syms * (N + CP), 1);
    end
