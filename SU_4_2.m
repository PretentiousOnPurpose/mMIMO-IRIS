...
%     DATA_SYMS_IND = [2,3,4,5,6,7,8];
    ...
    % Framing
    ul_tx_data_buff = zeros(2, N, numSyms - 1);
   
    iter_pilot_syms = 1;
    ul_tx_data_buff(1, nonZeroSubcarriers, 1:2:9) = 1;
    ul_tx_data_buff(2, nonZeroSubcarriers, 2:2:9) = 1;
      
    % OFDM
    ul_tx_data_buff1 = squeeze(circshift(ul_tx_data_buff(1, :, :), N/2));
    ul_tx_data_buff1 = ifft(ul_tx_data_buff1, N);
    ul_tx_data_buff1 = [ul_tx_data_buff1(end-CP+1: end, :); ul_tx_data_buff1];
   
    ul_tx_data_buff2 = squeeze(circshift(ul_tx_data_buff(2, :, :), N/2));
    ul_tx_data_buff2 = ifft(ul_tx_data_buff2, N);
    ul_tx_data_buff2 = [ul_tx_data_buff2(end-CP+1: end, :); ul_tx_data_buff2];

    ul_tx_data(1, :) = [zeros(1, iris_pre_zeropad), reshape(prmSeq, 1, 160), reshape(ul_tx_data_buff1(:), 1, numel(ul_tx_data_buff1)), 0.25 .* reshape(prmSeq, 1, 160), zeros(1, iris_post_zeropad)];
    ul_tx_data(2, :) = [zeros(1, iris_pre_zeropad), reshape(prmSeq, 1, 160), reshape(ul_tx_data_buff2(:), 1, numel(ul_tx_data_buff2)), 0.25 .* reshape(prmSeq, 1, 160), zeros(1, iris_post_zeropad)];
    ...
    if (SIM_MOD == 1)
        h = (1/sqrt(2)) * (randn(4, 2) + 1i * randn(4, 2));        
...
%     end   
    ...
    for iter_ant = 1:4
        ...
        % Channel Estimation
        h_hat(iter_ant, 1) = mean(mean(ul_rx_data_buff1(:, 1:2:9)));    
        h_hat(iter_ant, 2) = mean(mean(ul_rx_data_buff1(:, 2:2:9)));    
    end
