...
% Real HW:
    ...
    if ~isempty(hub_id)
        ...
        bs_param.n_sdrs = 1;
        bs_id1 = bs_param.id(1);
        bs_id2 = bs_param.id(2);
        bs_id3 = bs_param.id(3);
        bs_param.id = bs_id1;
        node_bs1 = iris_py(bs_param,[]);        % initialize BS
        bs_param.id = bs_id2;
        node_bs2 = iris_py(bs_param,[]);        % initialize BS
        bs_param.id = bs_id3;
        node_bs3 = iris_py(bs_param,[]);        % initialize BS
    end
    node_ue = iris_py(ue_param,[]);    % initialize UE

    node_ue.sdr_configgainctrl();
    
    node_bs1.sdrsync();                 % synchronize delays only for BS
    node_bs2.sdrsync();                 % synchronize delays only for BS
    node_bs3.sdrsync();                 % synchronize delays only for BS
    ...
    node_bs1.set_tddconfig(1, bs_param.tdd_sched(tdd_sched_index)); % configure the BS: schedule etc.
    node_bs2.set_tddconfig(1, bs_param.tdd_sched(tdd_sched_index)); % configure the BS: schedule etc.
    node_bs3.set_tddconfig(1, bs_param.tdd_sched(tdd_sched_index)); % configure the BS: schedule etc.
    node_ue.set_tddconfig(0, ue_param.tdd_sched(tdd_sched_index));

    node_bs1.sdr_setupbeacon();   % Burn beacon to the BS(1) RAM
    node_bs2.sdr_setupbeacon();   % Burn beacon to the BS(1) RAM
    node_bs3.sdr_setupbeacon();   % Burn beacon to the BS(1) RAM

    for i=1:n_ue
        node_ue.sdrtx_single(tx_data(:,i), i);       % Burn data to the UE RAM
    end
    node_bs1.sdr_activate_rx();          % activate reading stream
    node_bs2.sdr_activate_rx();          % activate reading stream
    node_bs3.sdr_activate_rx();          % activate reading stream
    ...
    [y1, data0_len1] = node_bs1.sdrrx(n_samp); % read data
    [y2, data0_len2] = node_bs2.sdrrx(n_samp); % read data
    [y3, data0_len3] = node_bs2.sdrrx(n_samp); % read data

    if (data0_len1 ~= data0_len2)
        data0_len = 0;
        return;
    end
    
    data0_len = data0_len1;
    
    y = [y1; y2; y3];
%     
    node_bs1.sdr_close();                % close streams and exit gracefully.
    node_bs2.sdr_close();                % close streams and exit gracefully.
    node_bs3.sdr_close();                % close streams and exit gracefully.
    node_ue.sdr_close();
